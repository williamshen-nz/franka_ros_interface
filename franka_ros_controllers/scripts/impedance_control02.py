...
if __name__ == '__main__':
    ...
    LCONTACT = 0.065
    MU_GROUND = 0.5
    IMPEDANCE_STIFFNESS_LIST = [1200, 1200, 1200, 100, 30, 100]
    NMAX = 20
    ...
    # excursion
    delta_theta, delta_s, delta_d, delta_t = 0.0, -0.03, 0., 10.

    # build impedance model
    obj_params = dict()
    obj_params['pivot'] = np.array([initial_pivot_xyz[0], initial_pivot_xyz[2]])
    obj_params['mgl'] = mgl
    ...
    param_dict['contact_pose_target'] = inital_generalized_positions
    ...
    while not rospy.is_shutdown():
            ...
            # print("impedance target: " + str(impedance_target))
    ...
    ax2[0].plot(np.mean(robot_friction_coeff_array)*np.array([0, -1.2*NMAX ]), 
        np.array([0, -1.2*NMAX]), color='k')
    ...
    ax2[1].plot(0.5*LCONTACT*np.array([0, -1.2*NMAX]), 
        np.array([0, -1.2*NMAX]), color='k')
    ax2[1].plot(-0.5*LCONTACT*np.array([0, -1.2*NMAX]), 
        np.array([0, -1.2*NMAX]), color='k')
