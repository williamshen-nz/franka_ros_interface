...
def detect_ee_apriltag():
    ...
    ee_apriltag_list = [detection for detection in apriltag_array.detections if detection.id == (0,)]
...
if __name__ == '__main__':
    ...
    # 1. initialize node
    rospy.init_node('ee_pose_in_world_from_camera',
        anonymous=True)
    ...
    ee_pose_in_world_from_camera_pub = rospy.Publisher('/ee_pose_in_world_from_camera_publisher', 
    ...
    tooltip_pose_apriltag_frame = list2pose_stamped([0,  -0.0555, -0.0121] + tooltip_quat_apriltag_frame.tolist())
    ...
    while not rospy.is_shutdown():
        ...
        ee_tooltip_in_world_pose = convert_reference_frame(tooltip_pose_apriltag_frame, base_in_base_pose,
                                                      ee_apriltag_in_world_pose, frame_id = "base")

        #5d. publish and sleep
        ee_pose_in_world_from_camera_pub.publish(ee_tooltip_in_world_pose)
        rate.sleep()