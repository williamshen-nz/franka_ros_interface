...
**************************************************************************/
#include <franka_ros_controllers/joint_position_controller.h>
...
bool JointPositionController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  desired_joints_subscriber_ = node_handle.subscribe(
      "/franka_ros_interface/motion_controller/arm/joint_commands", 20, &JointPositionController::jointPosCmdCallback, this,
  ...
  ROS_ERROR(
      "JointPositionController: Joint limits parameters not provided, aborting "
      "controller init!");
  return false;
      }
  ...
  for (size_t i = 0; i < joint_limits_.joint_names.size(); ++i){
    if (pos_limit_lower_map.find(joint_limits_.joint_names[i]) != pos_limit_lower_map.end())
      ...
      {
        ROS_ERROR("JointPositionController: Unable to find lower position limit values for joint %s...",
    ...
    if (pos_limit_upper_map.find(joint_limits_.joint_names[i]) != pos_limit_upper_map.end())
      ...
      {
        ROS_ERROR("JointPositionController: Unable to find upper position limit  values for joint %s...",
                       joint_limits_.joint_names[i].c_str());
      }
  }  
  ...
  if (!node_handle.getParam("controller_state_publish_rate", controller_state_publish_rate)) {
    ROS_INFO_STREAM("JointPositionController: Did not find controller_state_publish_rate. Using default "
...

  {
    ...
    publisher_controller_states_.msg_.controller_name = "joint_position_controller";
...
bool JointPositionController::checkPositionLimits(std::vector<double> positions)
{
  // bool retval = true;
  for (size_t i = 0;  i < 7; ++i){
...
void JointPositionController::jointPosCmdCallback(const franka_core_msgs::JointCommandConstPtr& msg) {
    ...
    if (msg->mode == franka_core_msgs::JointCommand::POSITION_MODE){
      if (msg->position.size() != 7) {
        ...
        pos_d_target_ = prev_pos_;
         ...
         ROS_ERROR_STREAM(
            "JointPositionController: Commanded positions are beyond allowed position limits.");
        pos_d_ = prev_pos_;
        pos_d_target_ = prev_pos_;
...
PLUGINLIB_EXPORT_CLASS(franka_ros_controllers::JointPositionController,
